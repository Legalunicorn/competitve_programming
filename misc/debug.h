...
inline void _debug_end() {
    std::cerr << '\n';
}

template <typename T>
inline void _debug_print(const char* name, T&& value) {
    std::cerr << name << " = " << value;
}

template <typename T, typename... Args>
inline void _debug_print(const char* names, T&& value, Args&&... args) {
    const char* comma = strchr(names, ',');
    std::cerr.write(names, comma - names) << " = " << value << " | ";
    _debug_print(comma + 1, std::forward<Args>(args)...);
}


#define debug(...) _debug_print(#__VA_ARGS__, __VA_ARGS__), _debug_end()

#else
#define debug(x)  // does nothing when LOCAL not defined
#endif
