...
void solve(){
    ...
    for (auto& z:a) cin >> z;
    map<ll,vi> pos;
    pos[0].pb(-1); // the default prefix sum sorta, debug leter to check if i should leave it as 0 
    pf[0] = a[0];
    pos[pf[0]].pb(0);
    for (int i=1; i<n;i++){
        pf[i] = pf[i-1]+a[i];
    ...
    for (int i = 0; i<n; i++){
        ...
        auto& v = pos[t];
        int dist = upper_bound(all(v),ft-1) - lower_bound(all(v), bk-1);
        // cerr << t << " ";
        // debugv(v);
        // cerr << bk << ":" << ft << endl;
        // cerr << i << " " << dist << endl;
