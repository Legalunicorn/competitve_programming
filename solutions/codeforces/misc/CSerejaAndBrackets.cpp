...
private:
    ...
    T combine(T l, T r){
        Node evl;
        evl.pair = l.pair + r.pair;
        evl.right = l.right;
        evl.left = r.left;
        int mx = min(l.left,r.right);
        evl.pair += mx;
        evl.left += (l.left-mx);
        evl.right += (r.right-mx);
        return evl;
    }

	void build(int low, int high, int pos, string& a){
		if (low==high){
            if (a[low]=='(') tree[pos] = Node(1,0,0);
...
public:
	...
	void build(string& a){ build(0,n-1,0,a); }
...
void solve(){
    ...
    while(m--){
        ...
        // cerr << ans.left << " : " << ans.right << " " << ans.pair << endl;
